...
class CartsDetail extends ServicePdo
{
    ...
    {
        $dbName = $this->dbName;
        $sql = "SELECT *, $dbName.id id  FROM $dbName JOIN products_detail ON products_detail.ID = $dbName.ID_PRODUCT_DETAIL JOIN products ON .ID = products_detail.ID_PRODUCT WHERE ID_CART = $idCart";
