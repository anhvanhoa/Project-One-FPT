...
<body>
    <div class="bg-gray-100">
        ...
        <div class="my-10 max-w-7xl mx-auto relative isolate bg-cover bg-bottom bg-[url('/asset/images/banner.jpg')] h-[500px] rounded-xl">
        ...
        <!-- component -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mt-16 px-2 max-w-screen-2xl mx-auto">
            <?php
            foreach ($vouchers as $voucher) {
                extract($voucher);
            ?>
                <div class="container mx-auto first:hidden last:hidden lg:first:block xl:last:block">
                    <div class="bg-gradient-to-br from-green-700 to-green-600/80 text-gray-100 text-center py-6 px-5 rounded-lg shadow-md relative">
                        ...
                        <div class="text-sm flex items-center justify-center space-x-2 mb-2">
                            <span id="cpnCode" class="border-dashed border text-gray-100 px-4 py-2 rounded-l uppercase"><?= $code ?></span>
                        </div>
                        <p class="text-xs">Từ <?= $start ?> || đến <?= $end ?></p>
                        <div class="w-12 h-12 bg-gray-100 rounded-full absolute top-1/2 transform -translate-y-1/2 left-0 -ml-6"></div>
                        <div class="w-12 h-12 bg-gray-100 rounded-full absolute top-1/2 transform -translate-y-1/2 right-0 -mr-6"></div>
                    </div>
                </div>
            <?php
            }
